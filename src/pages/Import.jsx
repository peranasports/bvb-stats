...
function Import() {
  const { loading, dispatch } = useContext(BVBStatsContext);
  ...
  const importPSBVBFiles = useCallback(async (files) => {
    for (const psbvbFile of files) {
      const m = initWithZippedBuffer(psbvbFile.buffer);
      dispatch({ type: "SET_LOADING", payload: {message: "Uploading " + m.matchDescription + "..."} });
      var matchInfo = getMatchInfo(m);
      matchInfo.psbvbFile = psbvbFile.buffer;
  ...
  const doImport = async () => {
    var sels = [];
    for (const psbvbFile of psbvbFiles) {
      if (psbvbFile.isSelected) {
        sels.push(psbvbFile);
      }
    }
    if (sels.length === 0) {
  ...
  const getMatchInfo = (match) => {
    var matchInfo = {
      ...
      tournament:
        match.tournament.name === undefined ? "" : match.tournament.name,
      venue: match.venue.name === undefined ? "" : match.venue.name,
      videoOffset: match.videoOffset,
      videoStartTime:
        match.videoStartTime === undefined
          ? new Date("01-01-1970")
          : match.videoStartTime,
    ...
    for (const pl of match.teamA.players) {
      matchInfo.teamA.players.push({
        ...
        positionString:
          pl.positionString === undefined ? "" : pl.positionString,
    ...
    for (const game of match.games) {
      matchInfo.games.push({
        ...
        startingLineup:
          game.startingLineup === undefined ? "" : game.startingLineup,
      });
    }
    return matchInfo;
  };
